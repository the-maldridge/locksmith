...
// to the network to actually assign the address to the peer.  The
...
type IPAM interface {
	NetInfo() NetInfo
	Assign(models.Peer) (net.IP, error)
	Release(models.Peer) (net.IP, error)
}

// A Factory returns a ready to use addresser
type Factory func(string) (IPAM, error)
...
func Initialize(name, pool string) (IPAM, error) {
	a, ok := impls[name]
	if !ok {
